...
from models.pc_two_trans import DynPredNet as TwoLevelNet
from models.pc_three import DynPredNet
import models.data_loader as data_loader



parser = argparse.ArgumentParser()
parser.add_argument('--data-dir', default='data/forest',
...
def train(model, optimizer, dataloader, params):
    ...
    with tqdm(total=len(dataloader), dynamic_ncols=True) as t:
        for i, train_batch in enumerate(dataloader):
            ...
            temp_loss, temp2_loss, r2_losses, _, _ = model(train_batch)
            # compute loss
            loss_dict = {
                "temp_loss": temp_loss.item(),
                "temp2_loss": temp2_loss.item(),
            }
            # clear previous gradients, compute gradients of all variables wrt loss
            for opt in optimizer: opt.zero_grad()
            loss = temp_loss + temp2_loss
            loss.backward()
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, scheduler, params, train_writer, test_writer, model_dir,
    ...
    for epoch in range(E):
        ...
        train_writer.add_scalar("Total", train_metrics['temp_loss'] + train_metrics['temp2_loss'], epoch)
        train_writer.add_scalar("Temporal Loss", train_metrics['temp_loss'], epoch)
        train_writer.add_scalar("Temporal2 Loss", train_metrics['temp2_loss'], epoch)
        ...
        test_writer.add_scalar("Total", test_metrics['temp_loss'] + test_metrics['temp2_loss'], epoch)
        test_writer.add_scalar("Temporal Loss", test_metrics['temp_loss'], epoch)
        test_writer.add_scalar("Temporal2 Loss", test_metrics['temp2_loss'], epoch)
        ...
        val_loss = test_metrics['temp_loss'] + test_metrics['temp2_loss']
...
if __name__ == '__main__':
    ...
    # load the first two level weights from previous model
    ckpt = utils.load_checkpoint("experiments/two_mnist_long/last.pth.tar", model)
    # load pretrained V2 
    clock = TwoLevelNet(params, device).to(device)
    ckpt = utils.load_checkpoint("experiments/two_trans_clock/last.pth.tar", clock)
    model.temporal2[0].load_state_dict(clock.temporal2.state_dict())
    straight = clock
    ckpt = utils.load_checkpoint("experiments/two_trans_straight/last.pth.tar", straight)
    model.temporal2[1].load_state_dict(straight.temporal2.state_dict())

    # only optimize new parameters
    optimizer = [
        optim.Adam(model.hypernet2.parameters(), params.learning_rate_t)
    ]
    scheduler = [ExponentialLR(opt, gamma=params.learning_rate_gamma) for opt in optimizer]

    # Train the model
    logging.info("Starting training for {} epoch(s)".format(params.num_epochs))
    train_and_evaluate(model, train_dl, val_dl, optimizer, scheduler, params, train_writer, 
                       test_writer, args.model_dir, args.restore_file)
