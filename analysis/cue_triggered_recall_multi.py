...
parser.add_argument('--data_dir', default='../data/memory_multi',
                    help="Directory containing the dataset")
parser.add_argument('--model_dir', default='../experiments/memory_multi',
...
def recall(memory, prednet, input_partial, mask, T=5):
    batch_size = input_partial.size(0)
    ...
    recalled_sequence = torch.zeros(batch_size, 5, 324)
    r = input_partial[:, :mask].to(device)
    R = torch.zeros(batch_size, T, r.size(1))
    for t in range(T):
        R[:,t] = r.clone().detach()
        img = prednet.spatial_decoder(r)
        recalled_sequence[:,t] = img.cpu().detach()
        r = prednet.temporal_prediction_(r, rh_recalled)
...
if __name__ == '__main__':
    ...
    prednet_fpath = "../experiments/two_mnist/"
    ...
    mask = prednet.r_dim
    # conditioning data
    R_conditioning = torch.tensor(np.load("../data/memory_multi/r_full_seq.npy"))
    params.batch_size = 4
    data_batch_start = train_dl.dataset.data[:]
    
    save_dir = op.join("results", "recall_multi", "trained")
    if not op.exists(save_dir):
        os.makedirs(save_dir)
    R_recalled_start = testing_recall(memory, prednet, R_conditioning, data_batch_start, save_dir)
    
    save_dir_raw = op.join("results", "recall_multi", "raw")
