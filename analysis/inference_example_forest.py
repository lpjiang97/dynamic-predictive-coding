...
parser.add_argument('--data_dir', default='../data/forest', help="Directory containing the forest dataset")
...
if __name__ == '__main__':
    ...
    np.savez(op.join(save_dir, f"result_dict_forest.npz"), **result_dict)
    
    # long term prediction
    result_dict = record(model, X, input_dim=params.input_dim, mixture=True, turnoff=3)
    np.savez(op.join(save_dir, f"result_dict_long_pred_forest.npz"), **result_dict)
