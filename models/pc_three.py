...
class DynPredNet(nn.Module):
    ...
    def __init__(self, params, device):
        ...
        self.temporal2 = nn.ModuleList([
            nn.Linear(params.r2_dim, params.r2_dim) for _ in range(params.mix_dim_2)
        ])
        self.hypernet2 = nn.Sequential(
            nn.Linear(params.r3_dim, params.hyper_hid_dim_2),
            nn.LayerNorm(params.hyper_hid_dim_2),
            nn.ELU(),
            nn.Linear(params.hyper_hid_dim_2, params.hyper_hid_dim_2),
            nn.Linear(params.hyper_hid_dim_2, self.mix_dim_2),
        )
        # hyperparams
        self.device = device
        self.r_dim = params.r_dim
        self.r2_dim = params.r2_dim
        self.r3_dim = params.r3_dim
        ...
        self.lr_r3 = params.lr_r3
        self.lmda_r3 = params.lmda_r3
    ...
    def forward(self, X):
        batch_size = X.size(0)
        T = X.size(1)
        r, r2, r3 = self.init_code_(batch_size)
        ...
        for t in range(2, T):
            ...
            r2_new, r3_new = self.inf_second(large_error_idx, r3.clone().detach(), r2.clone().detach(), r2_p, r_p, r)
            r2[large_error_idx] = r2_new.clone().detach()
            r3[large_error_idx] = r3_new.clone().detach()
            # learning
            r_hat = self.temporal_prediction_one_(r_p, r2)
            r2_hat = self.temporal_prediction_two_(r2_p, r3)
            # loss
    ...
    def inf_second(self, idx, r3, r2, r2_p, r_p, r):
        ...
        batch_size = r.size(0)
        r3 = r3[idx]
        r3.requires_grad = True
        r2 = r2[idx]
        r2.requires_grad = True
        optim = torch.optim.Adam([
            {'params': r2, 'lr': self.lr_r2},
            {'params': r3, 'lr': self.lr_r3, 'weight_decay:': self.lmda_r3}
        ])
        converged = False
        i = 0
        # inference
        while not converged and i < self.max_iter:
            old_r3 = r3.clone().detach()
            old_r2 = r2.clone().detach()
            r2_bar = self.temporal_prediction_two_(r2_p, r3)
            ...
            with torch.no_grad():
                converged = torch.norm(r2 - old_r2) / (torch.norm(old_r2) + 1e-16) < self.tol \
                        and torch.norm(r3 - old_r3) / (torch.norm(old_r3) + 1e-16) < self.tol
            i += 1
        self.converge_warning_(i, "r2/r3 did not converge")
        return r2.clone().detach(), r3.clone().detach()

    def inf(self, x, r_p, r2):
        batch_size = x.size(0)
        r, _, _ = self.init_code_(batch_size)
        r2.requires_grad = True
    ...
    def temporal_prediction_two_(self, r2, r3):
        outputs = 0
        w2 = self.hypernet2(r3)
        for i, mlp in enumerate(self.temporal2): 
            output = mlp(r2)
            outputs += w2[:,i:i+1] * output
        return outputs
    ...
    def init_code_(self, batch_size):
        ...
        r3 = torch.zeros((batch_size, self.r3_dim), requires_grad=True, device=self.device) 
        return r, r2, r3
